  ...
  return tool(
    {
      ...
      schema: z.object({
        sourceChain: z.string().describe('Name of the source chain sending the tokens (must exist in chainMap)'),
        destinationChain: z.string().describe('Name of the destination chain receiving the tokens (must exist in chainMap)'),
      ...
      func: async ({ sourceChain, destinationChain, amount, address }) => {
        try {
          ...
          if (sourceChainInfo.type === 'RelayChain' && destChainInfo.type === 'ParaChain') {
            ...
            throw new Error(`Unsupported transfer path: ${sourceChainInfo.type} to ${destChainInfo.type}`);
          }
