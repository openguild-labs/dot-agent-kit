...
async function attemptReconnection(endpoint: string): Promise<boolean> {
  ...
  if (connection.reconnectCount >= MAX_RECONNECT_ATTEMPTS) {
    console.error(
      `⛔ Maximum reconnection attempts (${MAX_RECONNECT_ATTEMPTS}) exceeded for ${endpoint}`,
    );
    return false;
  }
  ...
  console.warn(
    `⚠️ Connection error for ${endpoint}, reconnecting in ${delay}ms (attempt ${connection.reconnectCount + 1}/${MAX_RECONNECT_ATTEMPTS})...`,
  );
...
export async function substrateApi(
  config: SubstrateConnectorConfig,
  chain: Chain,
): Promise<{ api: any; disconnect: () => void }> {
  // Get the descriptor for this chain
  const descriptor = chainDescriptorRegistry.getDescriptor(chain);
  if (!descriptor) {
    throw new Error(
      `Chain descriptor not found for chain: ${chain}. Make sure to register it first.`,
    );
  }
