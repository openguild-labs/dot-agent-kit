...
async function main() {
  const { api, disconnect } = await substrateApi(
    { url: "wss://westend-rpc.polkadot.io", name: "westend" },
    "westend",
  );
  ...
  const myReceiverMultiAddress = toMultiAddress(
  ...
  const transferCall = await transferKeepAlive(
    api,
    myReceiverMultiAddress,
    BigInt(1),
  );
  console.log("Transfer call:", transferCall);

  /** 3. call as proxy with delegate signer **/
  const transferProxy = await callAsProxy(api, {
    address: myMultiAddress,
    call: transferCall,
  });
