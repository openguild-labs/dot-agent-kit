...
import { Api, getAllSupportedChains, getChainById, KnowChainId, parseUnits, getDecimalsByChainId} from "@polkadot-agent-kit/common"
import {
  getApiForChain,
  validateAndFormatMultiAddress,
  executeTool
} from "../utils"
...
export const transferNativeTool = (apis: Map<KnowChainId, Api<KnowChainId>>) => {
  return tool(
    async ({ amount, to, chain }: z.infer<typeof transferToolSchema>) => {
